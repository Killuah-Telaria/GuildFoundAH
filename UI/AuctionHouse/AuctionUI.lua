...
local TAB_REVIEWS = 4
local TAB_LFG = 5
local TAB_SETTINGS = 6
local TAB_DEATH_CLIPS = 7
local TAB_ATHENE = 8
...
StaticPopupDialogs["OF_SELECT_AUCTION_MONEY"] = {
    ...
    -- Keine automatische Erstellung des MoneyInputFrame, daher false
    hasMoneyInputFrame = false,

    OnShow = function(self)
        -- Überprüfen, ob der MoneyInputFrame bereits existiert
        if not self.moneyInputFrame then
            -- Manuellen MoneyInputFrame erstellen und das Template verwenden
            self.moneyInputFrame = CreateFrame("Frame", "MyMoneyInputFrame", self, "MoneyInputFrameTemplate")
            
            -- Setze die Position des MoneyInputFrame im Popup
            self.moneyInputFrame:SetPoint("TOP", self, "TOP", 0, -30)  -- Hier kannst du die Position anpassen

            -- Setze den Standardbetrag (0 Gold, 0 Silber, 0 Kupfer)
            MoneyInputFrame_SetAmount(self.moneyInputFrame, 0, 0, 0)
        end
    end,

    OnAccept = function(self)
        -- Wenn der Benutzer auf "Akzeptieren" klickt, den Wert des MoneyInputFrame auslesen
        local copper, silver, gold = GetMoney(self.moneyInputFrame)
        local totalAmount = gold * 10000 + silver * 100 + copper
        print("Der eingegebene Betrag ist: " .. totalAmount .. " Kupfer.")
    end,
...
function OFAuctionFrameAuctions_OnShow()
    OFAuctionsTitle:SetFormattedText(L["Guildfound AH - %s's Auctions"], UnitName("player"))
	OFAuctionsFrameAuctions_ValidateAuction()
	OFAuctionFrameAuctions_Update()
    OFPriceTypeDropdown:GenerateMenu()
    OFDeliveryDropdown:GenerateMenu()
